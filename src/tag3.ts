...
export function tag3(contentMap: JsonContent): string {
  ...
  for (const pageid in contentMap) {
    ...
    if (japanese) {
      ...
      const trimmed = he.decode(noBracket).replaceAll(/\（|\）|　/g, (match: string) => (
        match === "（"
          ? "("
          : match === "）"
            ? ")"
 ...
 */
export function tag3Creator(contentMap: JsonContent): string {
  /** pageid: japanese */
  const artistList: Map<number, string> = new Map();
  const groupList: Map<number, string> = new Map();
  /** pageid */
  const skippedList: Set<number> = new Set();
  /** pageid */
  const failureList: Set<number> = new Set();
