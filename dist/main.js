...
console.log("start Tag2");
{
    ...
    async function* readJsonFileGenerator(filePathList) {
...
//#endregion
//#region Tag3
import { tag3, tag3Creator, } from "./tag3.js";
console.log("start Tag3");
{
    for (const [p, t] of pathAndTitleList) {
        ...
        const contentMap = JSON.parse(data);
...
console.log("start Tag4");
{
    for (const [p, t] of pathAndTitleList) {
        if (t !== "Category:Creator_Tag") {
            ...
            const idTitleMap = JSON.parse(idTitleData);
            ...
            const idTranslateMap = JSON.parse(idTranslateData);
...
console.log("start final output");
{
    ...
    for (let i = 0; i < finalNameList.length; i++) {
        ...
        finalData[finalNameList[i]] = JSON.parse(data);
    }
